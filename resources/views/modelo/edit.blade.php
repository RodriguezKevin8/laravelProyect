    ...
    <x-authentication-card>
        ...
        <form method="POST" action="{{ route('modelos.update', $modelo->id) }}">
            ...
            <div>
                ...
                <x-input id="nombre" class="block mt-1 w-full" type="text" name="nombre" :value="old('nombre', $modelo->nombre)" required autofocus />
            </div>
            ...
            <div class="mt-4">
                ...
                <select id="id_marca" name="id_marca" class="block mt-1 w-full bg-gray-800 text-gray-200 border-gray-700 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm">
                    @foreach($marcas as $marca)
                        <option value="{{ $marca->id }}" {{ $modelo->id_marca == $marca->id ? 'selected' : '' }}>{{ $marca->marca }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="mt-4">
                ...
                <x-input id="anio" class="block mt-1 w-full bg-gray-800 text-gray-200 border-gray-700 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm" type="number" name="anio" :value="old('anio', $modelo->anio)" required />
            </div>
