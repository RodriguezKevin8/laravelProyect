    ...
    <x-authentication-card>
        ...
        <form method="POST" action="{{ route('proveedores.update', $proveedor->id) }}">
            ...
            <div>
                ...
                <x-input id="nombre_proveedor" class="block mt-1 w-full" type="text" name="nombre_proveedor" :value="old('nombre_proveedor', $proveedor->nombre_proveedor)" required autofocus />
            </div>

            <div class="mt-4">
                <x-label for="telefono" value="Teléfono" />
                <x-input id="telefono" class="block mt-1 w-full" type="text" name="telefono" :value="old('telefono', $proveedor->telefono)" required />
            </div>

            <div class="mt-4">
                <x-label for="correo" value="Correo Electrónico" />
                <x-input id="correo" class="block mt-1 w-full" type="email" name="correo" :value="old('correo', $proveedor->correo)" required />
            </div>

            <div class="mt-4">
                <x-label for="direccion" value="Dirección" />
                <x-input id="direccion" class="block mt-1 w-full" type="text" name="direccion" :value="old('direccion', $proveedor->direccion)" required />
            </div>
